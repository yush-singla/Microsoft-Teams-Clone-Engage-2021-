...
1. ReactJs is used to make the entire front end.
2. NodeJs is used for the server, it handles get/post requests through express and also handles socket events.
3. Socket.io is used for communication between the server and the client back and forth. It handles all the events like mute,cam off,etc and informs other peers in the meet to adjust the ui accordingly, thus maintaining a sync between all the users.
4. WebRTC is used to transfer video streams, Peer to Peer basis.
